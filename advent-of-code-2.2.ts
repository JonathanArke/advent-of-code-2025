...
function foundDuplicateIds(input: string[][]){
    let sumOfWrongIds: number = 0;

    input.forEach(el => {
        const rangeStart = Number(el[0])
        const rangeEnd = Number(el[1]);

        for(let i = rangeStart; i <= rangeEnd; i++){
            let currentNumber = i.toString();
            let halfLenghtOfCurrentNumber = currentNumber.length / 2

            for(let k = 1; k <= halfLenghtOfCurrentNumber; k++){
                if(allElementsAreEquals(chunkEveryDigit(currentNumber, k))){
                    sumOfWrongIds += i
    ...
    console.log("sum of invalid IDs", sumOfWrongIds)
}

function chunkEveryDigit(string:string, n = 1){
    let stringArr: string[] = [];

    for(let j = 0; j < string.length; j+=n){
        stringArr.push(string.substring(j, j + n))
    }

    return stringArr;
}

function allElementsAreEquals(arr: string[]){
    return Boolean(arr.every((el, i, arr) => el === arr[0]))
}


foundDuplicateIds(input)